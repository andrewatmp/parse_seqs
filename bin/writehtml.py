...
def make_taxa_table(input_file, sample_name):
    ...
    fig.add_trace(go.Table(
        header=dict(values=['<b>Species Name</b>', '<b>Relative Abundance (%)</b>'],
                    fill_color='paleturquoise',
                    align='center',
                    font=dict(size=20)),
        cells=dict(values=['<i>'+ df_transposed['Genus_Species_Limited'] + '</i>', df_transposed['Relative_Abundance'].round(1)],
                fill_color='white',
                align='center',
                font=dict(size=18))),
        row=1, col=1)
    ...
    fig.add_trace(go.Pie(labels='<i>'+ df_transposed['Genus_Species_Limited']+ '</i>', values=df_transposed['Relative_Abundance'],
                        textinfo='label+percent', hole=0.4, showlegend=False, textposition='outside',
                        texttemplate=labels_to_show, domain=domain_pie),
        row=2, col=1)

    # Update layout and title
    fig.update_layout(title='<b>Pathogens Detected for {sample}</b><br><br>Bacteria'.format(sample=sample_name))
    fig.update_layout(height=1000)

    return fig

# Modify the function to generate the complete HTML file content with modern title styling
def create_complete_html_modern_title(fig_html_str, logo_base64, title="MP Biomedicals Microbial ID Report", subtitle="{}".format(date.today())):
